          ...
          try {
            ...
            if (data.choices?.[0]?.delta?.reasoning_content) {
              if (reasoningCollected.length < MAX_REASONING_CHARS) {
                ...
                const snippet = data.choices[0].delta.reasoning_content.slice(0, remaining);
      ...
      const openaiResponse = {
        ...
        choices: response.data.choices.map(choice => {
          ...
          if (SHOW_REASONING && choice.message?.reasoning_content) {
            const reasoningSnippet = choice.message.reasoning_content.slice(0, MAX_REASONING_CHARS);
            fullContent = `<think>\n${reasoningSnippet}\n</think>\n\n` + fullContent;
          }
